...
<div id="main_wrapper">
		<div id="main" class="wrapper-register"><div class="padd10">
				...
				<form name="lostpass" action="<?php echo esc_url( get_site_url() . '/wp-login.php?action=lostpassword' ); ?>" method="post" id="loginform">
					...
					<div class='alert alert-warning'><?php _e('Please enter your information here. We will send you a new password.','ProjectTheme'); ?></div>
                    ...
                    <input type="text" class="form-control" name="user_login" id="user_login" placeholder="<?php _e('Username or Email:','ProjectTheme') ?>" value="" size="30" tabindex="1" />
                    </p>
					...
					<p><label>&nbsp;</label>
					<input type="submit" name="submit" id="submits" value="<?php _e('Retrieve Password','ProjectTheme'); ?>"  class="btn btn-primary btn-lg btn-block" tabindex="3" />
                    </p>
		...
		<?php
			...
			case 'rp' :
				...
				echo '<div class="container mt-4 mb-4"><div class="card">
            	<div class="p-3">';
			  ...
			  //Any differences will be noted with end of line comments.
				...
				if ( !empty($_POST['log']) && !force_ssl_admin() ) {
					...
					if ( $user = get_user_by('login', $user_name) ) {
							...
							if($active_user == "no" and $projectTheme_admin_approves_each_user == "yes")
							{
										$user1 			= new WP_Error( 'error_on_activation', __( "Please don't forget to confirm your e-mail address to start the activation process of your account." ) );
										$no_active 	= 1;
							}
					}
				...
				if ( empty( $_COOKIE[ LOGGED_IN_COOKIE ] ) ) {
					if ( headers_sent() ) {
						$user = new WP_Error( 'test_cookie', sprintf( __( 'Cookies are blocked due to unexpected output. For help, please see <a href="%1$s">this documentation</a> or try the <a href="%2$s">support forums</a>.','ProjectTheme' ),
							 'https://codex.wordpress.org/Cookies' ,   'https://wordpress.org/support/'   ) );
					} elseif ( isset( $_POST['testcookie'] ) && empty( $_COOKIE[ TEST_COOKIE ] ) ) {
						// If cookies are disabled we can't log in even with a valid user+pass
						$user = new WP_Error( 'test_cookie', sprintf( __( 'Cookies are blocked or not supported by your browser. You must <a href="%s">enable cookies</a> to use WordPress.' ,'ProjectTheme'),
...
<div class="container">
		<div id="main" class="wrapper-register">
            ...
            <div class="my_box4 card mt-4"><div class="padd25">
				...
				?>
                    ...
                    <div class="alert alert-warning"><div class="p-2">
                    <?php _e('We have sent a confirmation message to your email address.
					Please follow the instructions in the email. Check the spam folder too.','ProjectTheme'); ?>
                    </div></div>
				...
				<p>
				<input class="form-control" type="text" name="log" id="log" placeholder="<?php _e('Type your username','ProjectTheme'); ?>" value="<?php echo esc_html(stripslashes($user_login), 1); ?>" size="30"  />
                </p>


				<p>
				<input class="form-control" type="password" name="pwd" id="login_password" placeholder="<?php _e('Type your password','ProjectTheme'); ?>"  value="" size="30"  />
				</p>
                ...
                <?php do_action('login_form');
									...
									if(empty($redirect_to)) $redirect_to = get_permalink(get_option('ProjectTheme_my_account_page_id'));
